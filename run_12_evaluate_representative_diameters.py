...
def update_ae_cache(cache_file_path, required_files):
    ...
    for file_index, file_path in enumerate(required_files, start=1):
        if file_index == 1 or file_index % 100 == 0 or file_index == total_files:
            print(f"      checking AE file {file_index}/{total_files}", flush=True)
...
def build_exp2_dataset(dx_value, reagent_filter, trial_filter, cache_file):
    ...
    collected_data = []
    total_psd_files = len(all_psd_files)
    for psd_index, psd_file in enumerate(all_psd_files, start=1):
        if psd_index == 1 or psd_index % 25 == 0 or psd_index == total_psd_files:
            print(
                f"      matching particle-size file {psd_index}/{total_psd_files}",
                flush=True,
            )
        path_parts = psd_file.split(os.sep)
        if len(path_parts) < 3:
...
def train_exp2_models(dx_name, dx_value, output_dir, reagent_filter, trial_filter, cache_file):
    ...
    for current_reagent in np.unique(data_array[:, 4]):
        ...
        plt.ylabel(r"Total spectral power (a.u.)")
        plt.legend()
        ...
        plt.xlabel(r"Total spectral power (a.u.)")
...
def main():
    ...
    metrics_paths = []
    output_root = os.path.join("analysis_results", "run_12_evaluate_representative_diameters")
