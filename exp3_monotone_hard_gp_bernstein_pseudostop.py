...
Applies monotone_hard_gp_bernstein forward models (particle2ae, trained on exp2)
to exp3 AE time series, identifies pseudo-stop cycles via threshold crossing,
and produces per-(Material, Target) overlay plots plus a summary CSV.
...
def predict_monotone_mean(mean_model: dict, x: np.ndarray) -> np.ndarray:
    x = np.asarray(x, dtype=float).reshape(-1)
...
def main():
    ...
    print("--- Monotone Hard-GP(Bernstein) Threshold Pseudo-Stopping Analysis (exp3) ---")
    ...
    for material in materials:
        ...
        model_path = os.path.join(
            MODEL_DIR,
            f"monotone_hard_gp_bernstein_model_particle2ae_{model_key}_exp2.joblib",
        )
        if not os.path.exists(model_path):
            print(f"Warning: monotone_hard_gp_bernstein model not found for {material}: {model_path}")
            continue
        model = joblib.load(model_path)

        print(f"\n[{material}] monotone_hard_gp_bernstein model loaded.")
        ...
        for target_val in targets:
                    ...
                    rows.append(
                        {
                            ...
                            "Trial": trial,
                            "theta_hardgp_bernstein": theta_ae,
                            "theta_hardgp_bernstein_sigma": theta_sigma,
                            "reachable": False,
                            "cross_type": "unreach",
                            "k_star": None,
                            "S_AE_at_k_star": None,
            ...
            base_name = f"exp3_monotone_hard_gp_bernstein_pseudostop_{material}_{target_val}um"
    ...
    if rows:
        ...
        col_order = [
            ...
            "Target_D50",
            "Trial",
            "theta_hardgp_bernstein",
            "theta_hardgp_bernstein_sigma",
            "reachable",
            "cross_type",
            "k_star",
            "S_AE_at_k_star",
        ...
        csv_path = os.path.join(OUTPUT_DIR, "exp3_monotone_hard_gp_bernstein_pseudostop_summary.csv")
