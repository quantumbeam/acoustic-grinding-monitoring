...
```bash
uv run python run_01_train_particle_size_ae_model.py
uv run python run_02_validate_autonomous_stopping.py
uv run python run_03_plot_frequency_spectra.py
uv run python run_04_plot_ae_power_trends.py
...
```text
analysis_results
  run_01_train_particle_size_ae_model
  run_02_validate_autonomous_stopping
  run_03_plot_frequency_spectra
  run_04_plot_ae_power_trends
  run_05_plot_force_speed_dependence
```

## Executable Scripts

`run_01_train_particle_size_ae_model.py` trains monotone Bernstein polynomial models from the time-scheduled grinding trials and exports fitted models, metrics, model-validation files, and AE threshold values.

`run_02_validate_autonomous_stopping.py` validates the autonomous size-control stopping trials using the trained particle-size/AE models in the `run_01_train_particle_size_ae_model` output directory.

`run_03_plot_frequency_spectra.py` generates first/last AE frequency spectrum comparisons for the 25 min grinding trials.

`run_04_plot_ae_power_trends.py` generates AE total spectral power trends with the 4-point moving average.
