"""
Poly4 Threshold Pseudo-Stopping Visualization for exp3.

Applies degree-4 polynomial models (particle2ae, trained on exp2) to exp3 AE time series,
identifies pseudo-stop cycles via threshold crossing, and produces per-(Material, Target)
overlay plots plus a summary CSV.

Output: results/poly4/
"""
...
AE_SCALE_TO_MV2 = 1e6
MOVING_AVG_WINDOW = 4
OUTPUT_DIR = 'results/poly4'
MODEL_DIR = 'results/poly4'
...
def main():
    ...
    print("--- Poly4 Threshold Pseudo-Stopping Analysis (exp3) ---")
    ...
    for material in materials:
        ...
        # Load poly4 model (particle2ae)
        model_path = os.path.join(
            MODEL_DIR,
            f"poly4_model_particle2ae_{model_key}_exp2.joblib",
        )
        if not os.path.exists(model_path):
            print(f"Warning: poly4 model not found for {material}: {model_path}")
            continue
        ...
        print(f"\n[{material}] Poly4 model loaded. x_range: "
        ...
        for target_val in targets:
            # Compute threshold from poly4 (mV² units)
            theta_ae = float(poly(float(target_val)))
    ...
    if rows:
        ...
        col_order = [
            "Material", "Target_D50", "Trial", "theta_poly4", "reachable",
            "cross_type", "k_star", "S_AE_at_k_star", "min_S_AE", "margin",
            "n_cycles", "Measured_D50",
        ]
        df = df[[c for c in col_order if c in df.columns]]
        csv_path = os.path.join(OUTPUT_DIR, "exp3_poly4_pseudostop_summary.csv")
