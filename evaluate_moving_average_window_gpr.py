...
def select_window(results, rmse_tol):
    ...
    # RMSE-based recommendation: among near-best RMSE windows, pick the simplest (smallest N).
    return min(candidates, key=lambda r: (r["window_size"], r["nlpd"]))
...
def main():
    ...
    for cv_method in cv_methods:
        for reagent in reagent_names:
            ...
            if args.plot_metrics and results:
                ...
                axes[1].plot(windows, nlpd_vals, "s-", color="tab:blue")
                ...
                fig.tight_layout()
                base = f"moving_average_cv_{args.experiment}_{cv_method}_{reagent}"
    ...
    summary_path = os.path.join(
        args.out_dir, f"moving_average_cv_{args.experiment}_{args.cv}.csv"
    )
    pd.DataFrame(summary_rows).to_csv(summary_path, index=False)
    print(f"Saved CV summary: {summary_path}")

    rec_path = os.path.join(
        args.out_dir, f"moving_average_cv_recommended_{args.experiment}_{args.cv}.csv"
    )
