...
"""
GPR Threshold Pseudo-Stopping Visualization for exp3.

Applies GPR forward models (particle2ae, trained on exp2) to exp3 AE time series,
identifies pseudo-stop cycles via threshold crossing, and produces per-(Material, Target)
overlay plots plus a summary CSV.

Output: model_comparison/gpr/
"""
...
AE_SCALE_TO_MV2 = 1e6
MOVING_AVG_WINDOW = 4
OUTPUT_DIR = os.path.join('model_comparison', 'gpr')
RESULTS_DIR = os.path.join('model_comparison', 'gpr')
K_SIGMA_AE = 0.0  # threshold offset: theta = mu - K * sigma
...
def main():
    ...
    print("--- GPR Threshold Pseudo-Stopping Analysis (exp3) ---")
    ...
    for material in materials:
        ...
        # Load GPR forward model (particle2ae)
        model_path = os.path.join(
            RESULTS_DIR,
            f"gpr_model_particle2ae_{model_key}_exp2.joblib",
        )
        if not os.path.exists(model_path):
            print(f"Warning: GPR model not found for {material}: {model_path}")
            continue
        gpr = joblib.load(model_path)

        print(f"\n[{material}] GPR model loaded.")
        ...
        for target_val in targets:
            # Compute threshold from GPR (mV² units)
            x_target = np.array([[float(target_val)]], dtype=float)
            ae_mu, ae_std = gpr.predict(x_target, return_std=True)
            theta_ae = float(ae_mu[0]) - K_SIGMA_AE * float(ae_std[0])
            theta_sigma = float(ae_std[0])
            ...
            base_name = f"exp3_gpr_pseudostop_{material}_{target_val}um"
    ...
    if rows:
        ...
        col_order = [
            "Material", "Target_D50", "Trial", "theta_gpr", "theta_gpr_sigma",
            "reachable", "cross_type", "k_star", "S_AE_at_k_star", "min_S_AE",
            "margin", "n_cycles", "Measured_D50",
        ]
        df = df[[c for c in col_order if c in df.columns]]
        csv_path = os.path.join(OUTPUT_DIR, "exp3_gpr_pseudostop_summary.csv")
