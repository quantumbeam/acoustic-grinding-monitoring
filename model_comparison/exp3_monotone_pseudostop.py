...
"""
Monotone Threshold Pseudo-Stopping Visualization for exp3.

Applies isotonic+PCHIP monotone models (trained on exp2) to exp3 AE time series,
...
AE_SCALE_TO_MV2 = 1e6
MOVING_AVG_WINDOW = 4
OUTPUT_DIR = os.path.join('model_comparison', 'monotonic')
RESULTS_DIR = os.path.join('model_comparison', 'monotonic')
...
# ============================================================
# Monotone model helpers (from exp2_isotonic_monotone_plot.py)
# ============================================================
def predict_pchip(
...
def main():
    ...
    print("--- Monotone Threshold Pseudo-Stopping Analysis (exp3) ---")
    ...
    for material in materials:
        ...
        # Load monotone model (particle2ae direction)
        model_path = os.path.join(
            RESULTS_DIR,
            f"isotonic_pchip_model_particle2ae_{model_key}_exp2.joblib",
        )
        if not os.path.exists(model_path):
            print(f"Warning: monotone model not found for {material}: {model_path}")
            continue
        model = joblib.load(model_path)
        pchip = model["pchip"]
        x_knots = model["x_knots"]
        y_knots = model["y_knots"]
        extrapolate = model.get("extrapolate", False)

        print(f"\n[{material}] Model loaded. x_knots range: "
              f"[{x_knots.min():.1f}, {x_knots.max():.1f}] um")
        ...
        for target_val in targets:
            # Compute threshold from monotone model (mV² units)
            theta_ae = float(
                predict_pchip(
                    pchip,
                    np.array([float(target_val)]),
                    x_knots,
                    y_knots,
                    extrapolate,
                )[0]
            ...
            for trial in ['1st', '2nd', '3rd']:
                ...
                #   last_cross    = threshold crossed between second-to-last and last smoothed value
                #   earlier_cross = threshold was already crossed before the last step
                #   unreach       = never crossed
                if not reachable:
                    cross_type = "unreach"
            ...
            for trial in ['1st', '2nd', '3rd']:
                ...
                # x-axis: raw cycle index corresponding to each smoothed value
                ...
                if td["reachable"]:
                    ...
                    # Annotate "unreach" near the end of the curve
                    ax.annotate(
                        "unreach",
            ...
            base_name = f"exp3_pseudostop_{material}_{target_val}um"
    ...
    if rows:
        ...
        col_order = [
            "Material", "Target_D50", "Trial", "theta_mono", "reachable",
            "cross_type", "k_star", "S_AE_at_k_star", "min_S_AE", "margin",
            "n_cycles", "Measured_D50",
        ]
        df = df[[c for c in col_order if c in df.columns]]
        csv_path = os.path.join(OUTPUT_DIR, "exp3_pseudostop_summary.csv")
