...
from monotone_ispline import ISplineConfig, MonotoneISplineRegressor
...
def build_monotone_mean(
    x_data: np.ndarray,
    y_data: np.ndarray,
    monotone: str,
    degree: int,
    n_internal_knots: int,
) -> MonotoneISplineRegressor:
    cfg = ISplineConfig(
        degree=degree,
        n_internal_knots=n_internal_knots,
        monotone=monotone,
    )
    return MonotoneISplineRegressor(cfg).fit(x_data, y_data)


def predict_monotone_mean(mean_model: MonotoneISplineRegressor, x: np.ndarray) -> np.ndarray:
    x = np.asarray(x, dtype=float).reshape(-1, 1)
    return np.asarray(mean_model.predict(x), dtype=float).reshape(-1)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train monotone hard-mean + residual GP models on AE and PSD data (both directions)."
    )
    ...
    dataset_path = os.path.join(output_dir, f"{experiment}_monotone_hard_gp_ispline_dataset_raw.csv")
    ...
    for current_reagent in np.unique(data_array[:, 4]):
        ...
        for direction, x_raw, y_raw in settings:
            ...
            if args.constraint == "auto":
                monotone_direction = infer_monotone_direction(x_data, y_data)
            else:
                monotone_direction = args.constraint

            mean_model = build_monotone_mean(
                x_data,
                y_data,
            ...
            model_path = os.path.join(
                output_dir,
                f"monotone_hard_gp_ispline_model_{direction}_{current_reagent}_{experiment}.joblib",
            )
            ...
            r2_total = float(1.0 - np.sum((y_data - y_train_pred_total) ** 2) / np.sum((y_data - np.mean(y_data)) ** 2))
            ...
            all_metrics.append(
                {
                    ...
                    "constraint_direction": monotone_direction,
                    "ispline_degree": int(args.degree),
                    "ispline_n_internal_knots": int(args.n_internal_knots),
            ...
            plt.plot(x_plot, y_plot_mean_mono, "k--", label="Monotone mean (hard, I-spline)")
            ...
            plt.legend()
            plot_png = os.path.join(output_dir, f"{experiment}_monotone_hard_gp_ispline_plot_{direction}_{current_reagent}.png")
    ...
    metrics_path = os.path.join(output_dir, f"{experiment}_monotone_hard_gp_ispline_metrics_both_directions.csv")
